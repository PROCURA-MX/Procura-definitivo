...
// Función para obtener el RFC de la organización
async function getOrganizationRFC(): Promise<string> {
  try {
    const token = localStorage.getItem('token');
    if (!token) {
      console.warn('No hay token disponible, usando RFC por defecto');
      return 'E0005'; // RFC por defecto
    }

    const response = await fetch('/api/organizaciones/current', {
      headers: {
    ...
    if (response.ok) {
      ...
      return organizacion.rfc || 'E0005'; // RFC de la organización o por defecto
    }
  } catch (error) {
    console.error('Error obteniendo RFC de organización:', error);
  }
  
  return 'E0005'; // RFC por defecto si hay error
}
      ...
      if (data.pidioFactura) {
        try {
          ...
          const ticketData = {
            ...
            "NumTicket": folio,
            "ClaveTienda": await getOrganizationRFC(),
