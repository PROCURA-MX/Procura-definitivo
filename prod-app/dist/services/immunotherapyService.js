...
exports.ImmunotherapyService = void 0;
...
class ImmunotherapyService {
    ...
    async createOrUpdateRecord(dto) {
    ...
    async getPatientRecord(pacienteId, organizacionId) {
    ...
    async getImmunotherapyLog(pacienteId, organizacionId) {
     ...
     */
    async getLastTreatmentForPatient(pacienteId, organizacionId) {
        try {
    ...
    async getRecordWithLog(pacienteId, organizacionId) {
        const record = await this.getPatientRecord(pacienteId, organizacionId);
        if (!record)
            return null;
        // 🎯 PROCESAR Y ENRIQUECER LOS LOG ENTRIES
        const enrichedLogEntries = (record.logs || []).map((entry) => {
        ...
        const todosLosAlergenos = new Set();
        log.forEach((entry) => {
            if (entry.alergenos && Array.isArray(entry.alergenos)) {
                entry.alergenos.forEach((alergeno) => {
        ...
        const todasLasReacciones = new Set();
        log.forEach((entry) => {
    ...
    async updateStartDate(recordId, nuevaFecha, editadoPor) {
    ...
    async updateReaction(usageId, tuvoReaccion, descripcion) {
    ...
    async hasRecord(pacienteId, organizacionId) {
    ...
    async getAllRecords(organizacionId) {
    ...
    parseAndValidateDate(dateValue) {
        if (!dateValue) {
...
exports.ImmunotherapyService = ImmunotherapyService;
