...
router.get('/mis-permisos', tenantMiddleware_1.authenticateMultiTenant, permisosController_1.obtenerMisPermisos);
// GET /usuarios - Obtener lista de usuarios (para gestión de permisos)
router.get('/usuarios', tenantMiddleware_1.authenticateMultiTenant, async (req, res) => {
    try {
        ...
        const usuarios = await prisma_1.default.$queryRaw `
      SELECT 
        u.id, 
        u.nombre, 
        u.apellido, 
...
router.put('/usuarios/:userId/permisos', tenantMiddleware_1.authenticateMultiTenant, async (req, res) => {
    try {
...
const CreateUserSchema = zod_1.z.object({
    nombre: zod_1.z.string().min(1, 'El nombre es requerido').max(100, 'El nombre es demasiado largo'),
    apellido: zod_1.z.string().min(1, 'El apellido es requerido').max(100, 'El apellido es demasiado largo'),
    email: zod_1.z.string().email('Email inválido').max(255, 'El email es demasiado largo'),
    telefono: zod_1.z.string().min(10, 'El teléfono debe tener al menos 10 dígitos').max(20, 'El teléfono es demasiado largo'),
    password: zod_1.z.string().min(6, 'La contraseña debe tener al menos 6 caracteres').max(255, 'La contraseña es demasiado larga'),
    rol: zod_1.z.enum(['DOCTOR', 'SECRETARIA', 'ENFERMERA']),
...
router.post('/usuarios', tenantMiddleware_1.authenticateMultiTenant, async (req, res) => {
    try {
