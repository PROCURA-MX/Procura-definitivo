...
exports.registerInventoryExit = void 0;
exports.getInventoryEntriesByCategory = getInventoryEntriesByCategory;
exports.getInventoryExitsByCategory = getInventoryExitsByCategory;
exports.getInventoryEntriesDetailByCategory = getInventoryEntriesDetailByCategory;
exports.getInventoryExitsDetailByCategory = getInventoryExitsDetailByCategory;
...
const registerInventoryExit = async (req, res) => {
    ...
    try {
        ...
        console.log(`🔍 [${requestId}] Consultorio info:`, {
            ...
            userConsultorioId: req.userConsultorioId,
            finalConsultorioId: req.body.consultorioId || req.userConsultorioId || null
        });
        ...
        const invalidItems = req.body.items.filter((item) => !item.productId);
        ...
        const adaptedData = {
            ...
            consultorioId: req.body.consultorioId || req.userConsultorioId || null, // Usar el consultorio del formulario o el del usuario
        ...
        const result = await (0, inventoryService_1.processInventoryUsage)(adaptedData);
...
exports.registerInventoryExit = registerInventoryExit;
// Endpoint para obtener entradas de inventario agrupadas por categoría
async function getInventoryEntriesByCategory(req, res) {
    try {
        ...
        const sedeId = querySedeId || req.tenantId || 'sede-organizacion-default';
        ...
        const entries = await prisma_1.default.movement.findMany({
            where,
            select: {
                quantity: true,
                totalCost: true,
...
async function getInventoryExitsByCategory(req, res) {
    try {
        const sedeId = req.sedeId;
        ...
        const groupedData = movements.reduce((acc, movement) => {
            ...
            const existingProduct = acc[category].products.find((p) => p.id === product.id);
            if (existingProduct) {
...
async function getInventoryEntriesDetailByCategory(req, res) {
    try {
        ...
        const sedeId = req.tenantId || 'sede-organizacion-default';
        let from = typeof req.query.from === 'string' ? req.query.from : undefined;
        let to = typeof req.query.to === 'string' ? req.query.to : undefined;
        if (!category) {
        ...
        const consultorios = await prisma_1.default.consultorio.findMany({
...
async function getInventoryExitsDetailByCategory(req, res) {
    try {
        ...
        const exits = await prisma_1.default.movement.findMany({
            where: {
                ...where,
                // 🎯 FILTRO MULTI-TENANT: Solo movimientos de productos de la organización actual
                Product: {
