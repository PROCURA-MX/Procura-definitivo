...
exports.processInventoryUsage = processInventoryUsage;
const library_1 = require("@prisma/client/runtime/library");
const prisma_1 = __importDefault(require("../prisma"));
const eventService_1 = require("./eventService");
// import { registrarHistorialInventario } from '../utils/historialUtils';
const treatmentUtils_1 = require("../utils/treatmentUtils");
const robust_product_service_1 = require("../robust-product-service");
// 🎯 FUNCIÓN PARA OBTENER VALOR DINÁMICO DE ENTRADA MÁS RECIENTE
async function getLatestEntryUnitCost(tx, productId, sedeId) {
    try {
...
async function getAlergenIdByName(tx, alergenoNombre, organizationId, sedeId) {
    try {
        const result = await (0, robust_product_service_1.findProductForTreatment)(tx, alergenoNombre, organizationId, sedeId || '');
...
async function getDiluyenteIdByName(tx, diluyenteNombre) {
...
async function processInventoryUsage(dto) {
...
async function processComplexTreatment(tx, dto, item, inventoryUsageId) {
...
async function processGlicerinadoTreatment(tx, dto, item, inventoryUsageId, product) {
...
async function processGlicerinadoPorUnidad(tx, dto, item, inventoryUsageId) {
    ...
    const { calculateGlicerinadoPorUnidad } = await Promise.resolve().then(() => __importStar(require('../utils/treatmentUtils')));
...
async function processGlicerinadoPorFrasco(tx, dto, item, inventoryUsageId) {
    console.log('🔄 Procesando Glicerinado por Frasco');
    const doses = item.doses || 1;
    const frascoLevels = Array.isArray(item.frascoLevels)
        ? item.frascoLevels
        : item.frascoLevel !== undefined
            ? [item.frascoLevel]
            : [];
    const allergenIds = item.allergenIds || [];
...
async function processAlxoidTreatment(tx, dto, item, inventoryUsageId, product) {
    ...
    if (subtipo === 'ALXOID_A')
        tipoReal = 'A';
    else if (subtipo === 'ALXOID_B')
        tipoReal = 'B';
    else if (subtipo === 'ALXOID_B_2' || subtipo === 'ALXOID_B.2')
        tipoReal = 'B.2'; // 🎯 CORREGIDO: Reconocer ambos formatos
    else
        tipoReal = 'A'; // Default
    console.log('🎯 Subtipo mapeado:', { subtipo, tipoReal });
    // Calcular componentes del tratamiento
    const { calculateAlxoidTreatment } = await Promise.resolve().then(() => __importStar(require('../utils/treatmentUtils')));
...
async function processAllergenMovement(tx, dto, allergenIdOrName, quantity, inventoryUsageId) {
    ...
    if (!product) {
        ...
        const allergen = allAllergens.find((a) => normalizeString(a.name) === normalizedInput);
        if (allergen) {
            product = allergen;
        }
    }
...
async function processDiluentMovement(tx, dto, diluentName, quantity, inventoryUsageId) {
...
async function processSimpleProduct(tx, dto, item, inventoryUsageId) {
    ...
    if ((0, treatmentUtils_1.isTreatment)(product.name)) {
    ...
    if (!stock || stock.quantity < requestedQuantity) {
        ...
        const productsWithStock = alternativeProducts.filter((p) => p.StockBySede.length > 0);
    ...
    const quantity = new library_1.Decimal(item.cantidad || item.quantity || 1);
    // Logging para debug
...
async function processTreatmentProduct(tx, dto, item, inventoryUsageId, product) {
...
async function processMovement(tx, dto, product, quantity, inventoryUsageId) {
    console.log(`🔄 Processing movement for product: ${product.name}, quantity: ${quantity}`);
    const quantityDecimal = typeof quantity === 'number' ? new library_1.Decimal(quantity) : quantity;
...
async function validateStock(sedeId, productId, quantity) {
    const stock = await prisma_1.default.stockBySede.findUnique({
        where: {
            productId_sedeId: {
                productId,
                sedeId,
...
async function getStockWithExpiry(sedeId, productId) {
    return prisma_1.default.productExpiration.findFirst({
        where: {
            productId,
            sedeId,
            quantity: { gt: 0 },
...
async function processSublingualTreatment(tx, dto, item, inventoryUsageId) {
    ...
    // Calcular componentes del tratamiento
    const { calculateSublingualTreatment } = await Promise.resolve().then(() => __importStar(require('../utils/treatmentUtils')));
...
async function processGlicerinadoEnFrasco(tx, dto, item, inventoryUsageId) {
    ...
    // Calcular componentes del tratamiento
    const { calculateGlicerinadoEnFrasco } = await Promise.resolve().then(() => __importStar(require('../utils/treatmentUtils')));
