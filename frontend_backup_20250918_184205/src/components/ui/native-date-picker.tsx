...
export function NativeDatePicker({
  ...
  const handleDateChange = useCallback((date: Date) => {
    // Delay para asegurar que el evento se procese
    setTimeout(() => {
      onChange(date)
      setIsOpen(false)
    }, 50)
  ...
  // SOLUCIÓN ROBUSTA: Sistema de eventos completamente controlado
  useEffect(() => {
    if (!isOpen) return
    ...
    const handleScroll = () => {
      // Cerrar al hacer scroll para evitar problemas de posicionamiento
      setIsOpen(false)
    }
              ...
              {enableQuickNavigation && (
                <div className="flex items-center gap-2 mt-2">
                  <select
                    ...
                    className="px-2 py-1 text-sm bg-gray-600 text-white rounded border border-gray-500 focus:outline-none focus:ring-1 focus:ring-gray-400 cursor-pointer"
                  >
