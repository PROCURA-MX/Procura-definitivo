...
import type { AppointmentReminderRequest } from '../types/express'
...
O responde "AGENDAR" para que te contactemos.`
   ...
   */
  static async sendTreatmentReminder(data: any): Promise<WhatsAppResponse> {
    return this.sendWeeklyTreatmentReminder(
      data.pacienteId || data.id,
      data.treatmentType || 'tratamiento',
      data.usuarioId || data.userId
    );
  }
   ...
   */
  static async sendPaymentNotification(data: any): Promise<WhatsAppResponse> {
    try {
      // Verificar si WhatsApp está configurado
      if (!PHONE_NUMBER_ID || !ACCESS_TOKEN) {
        throw new Error('WhatsApp Business API no configurado')
...
${data.fechaVencimiento ? `Fecha de vencimiento: ${format(data.fechaVencimiento, "dd/MM/yyyy")}` : ''}
