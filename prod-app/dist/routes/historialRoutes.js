...
async function formatAndAddHistorial(historialCompleto, historialCobros, historialInventario, historialCalendario, historialUsuarios) {
    // Formatear cobros
...
function getActionDescription(tipoCambio, modulo) {
    switch (tipoCambio) {
        case 'CREACION':
...
router.get('/general', tenantMiddleware_1.authenticateMultiTenant, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        if (!modulo || modulo === 'todos') {
            ...
            const historialCobros = await prisma_1.default.historialCobro.findMany({
                where: {
            ...
            const historialInventario = await prisma_1.default.movement.findMany({
                where: {
                    organizacion_id: organizacionId
                },
            ...
            const historialCalendario = await prisma_1.default.citas.findMany({
                where: {
                    consultorios: {
                        organizacion_id: organizacionId
                    }
            ...
            const historialUsuarios = await prisma_1.default.usuario.findMany({
        ...
        console.log(`🔍 DEBUG - Historial paginado: ${Math.min(historialCompleto.length, parseInt(limit))} registros`);
        res.json({
            success: true,
            ...
            pagination: {
                ...
                pages: Math.ceil(historialCompleto.length / parseInt(limit))
            }
        });
    }
    catch (error) {
...
router.get('/estadisticas', tenantMiddleware_1.authenticateMultiTenant, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    try {
        const organizacionId = req.organizationId;
        if (!organizacionId) {
            return res.status(400).json({
        ...
        const totalCobros = await prisma_1.default.historialCobro.count();
        const cobrosHoy = await prisma_1.default.historialCobro.count({
            where: {
                created_at: {
        ...
        const cobrosEstaSemana = await prisma_1.default.historialCobro.count({
            where: {
                created_at: {
        ...
        const usuariosActivos = await prisma_1.default.historialCobro.findMany({
            select: {
                usuario_id: true,
                usuario: {
                    select: {
...
router.get('/buscar', tenantMiddleware_1.authenticateMultiTenant, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        const cobros = await prisma_1.default.historialCobro.findMany({
            where: {
                OR: [
                    { tipo_cambio: { equals: query } }
                ]
            },
            take: parseInt(limit),
            include: {
...
router.get('/inventario', tenantMiddleware_1.authenticateMultiTenant, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    try {
...
router.get('/calendario', tenantMiddleware_1.authenticateMultiTenant, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    try {
        const { limit = '50', offset = '0' } = req.query;
        const organizacionId = req.organizationId;
...
router.get('/usuarios', tenantMiddleware_1.authenticateMultiTenant, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    try {
        const { limit = '50', offset = '0' } = req.query;
        const organizacionId = req.organizationId;
