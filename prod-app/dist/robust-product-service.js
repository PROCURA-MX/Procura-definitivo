...
exports.RobustProductService = void 0;
exports.findProductForTreatment = findProductForTreatment;
/**
...
class RobustProductService {
     ...
     */
    async findProductByName(productName, organizationId, sedeId) {
        try {
     ...
     * 3. Búsqueda parcial
     */
    async findProductAdvanced(productName, organizationId, sedeId) {
     ...
     */
    async validateStock(productId, sedeId, requiredQuantity) {
     ...
     */
    async findProductsWithStockInSede(sedeId, organizationId, category) {
        const whereClause = {
            stockBySede: {
                some: {
                    sedeId: sedeId,
...
exports.RobustProductService = RobustProductService;
/**
 ...
 */
async function findProductForTreatment(prisma, productName, organizationId, sedeId) {
    const service = new RobustProductService(prisma);
    const result = await service.findProductAdvanced(productName, organizationId, sedeId);
    if (!result.product) {
        throw new Error(result.error || `Producto "${productName}" no encontrado`);
    }
    // Validar stock
    const stockValidation = await service.validateStock(result.product.id, sedeId, 0.01 // Mínimo requerido
    );
