...
export default function InventoryEntryPage() {
  ...
  const handleCategoryClick = useCallback(async (category: string, totalQuantity: number, totalValue: number) => {
    try {
      ...
      if (selectedConsultorioId === 'todos') {
        url = `/api/inventory/inventory-entry/detail/by-category?category=${encodeURIComponent(category)}&allConsultorios=true&from=${from}&to=${to}`
      } else {
        ...
        url = `/api/inventory/inventory-entry/detail/by-category?category=${encodeURIComponent(category)}&sedeId=${consultorioSedeId}&from=${from}&to=${to}`
      }
